...
ShapeRecognizer::ShapeRecognizer(cv::Mat source)
{
    _inputImage = source;
}

/*Метод для отыскания геометрических фигур на изображении*/
void ShapeRecognizer::findGeometricalFeatures()
{
...
bool ShapeRecognizer::isTriangle(std::vector<cv::Point> &contour)
{
    return (contour.size() < 5);
}

/*Метод для определения, является ли контур прямоугольником */
bool ShapeRecognizer::isRectangle(std::vector<cv::Point> & contour)
{
...
bool ShapeRecognizer::isRound(std::vector<cv::Point> & contour)
{
    if(contour.size() > 6)
    {
...
double ShapeRecognizer::angle(cv::Point pt1, cv::Point pt2, cv::Point pt0)
{
...
QList<std::vector<cv::Point> > ShapeRecognizer::getRectangles()
{
    QList<std::vector<cv::Point> > list = _rectangles;
    return list;
}

QList<std::vector<cv::Point> > ShapeRecognizer::getTriangles()
{
    QList<std::vector<cv::Point> > list = _triangles;
    return list;
}

QList<std::vector<cv::Point> > ShapeRecognizer::getRounds()
{
